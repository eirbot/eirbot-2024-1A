// This file handles the connection to the motors and any low-level
// commands that must be sent to the motors

void set_wheels_rotation_from_global_rotation(float theta_rad, float *wheel_left_rotation, float *wheel_right_rotation);
void set_wheels_rotation_from_distance(float theta_rad, float *wheel_left_rotation, float *wheel_right_rotation);
void accomplish_steps(float *wheel_left_rotation, float *wheel_right_rotation);