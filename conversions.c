...
void command(char avancer_ou_tourner[], float consigne_distance_ou_rotation)
{
	if ("avance")
	{
		float distance = consigne_distance_ou_rotation;
		float consigne_moteur_avancer = consigne_moteur_from_longueur(distance); //fonction qui prend en argument la distance que doit parcourir le robot et renvoie l'angle (ou nombre de pas) que doit faire les moteur avec l'hypothese teta_gauche = teta_droite
	...
	{
		float angle_rotation = consigne_distance_ou_rotation;
...
float consigne_moteur_gauche(float consigne)
{}
